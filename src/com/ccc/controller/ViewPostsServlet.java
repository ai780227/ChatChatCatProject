...
public class ViewPostsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			String m_id = "";
			int page= 0, size= 0, post_type= 0, post_property = 0;
			List<PostBean> lpb;
...
//			req.setAttribute("userId", userId);
			...
			if (req.getParameter("m_id") != "") {
				m_id = req.getParameter("m_id");
				lpb = ps.viewUserPosts(mb, m_id, page, size);
			} else if (req.getParameter("post_type") == "" && req.getParameter("post_property") == "") {
				lpb = ps.viewAllPosts(mb, page, size);
			} else {
				post_type = Integer.parseInt(req.getParameter("post_type"));
				post_property = Integer.parseInt(req.getParameter("post_property"));
				pb = new PostBean();
				pb.setPost_type(post_type);
				pb.setPost_property(post_property);
				lpb = ps.viewPostsByTags(pb, mb, page, size);
			}
