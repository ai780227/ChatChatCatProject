	...
	$.ajax({
		...
		"success" : function(data) {
			if(data != "") {
				var json = eval('(' + data + ')');
				if (json.posts != "") {
					if (json.post_size < size) {
						length = json.post_size;
					} else {
						length = size;
					}
					for(var i = 0; i < length; i++) {
						if (json.posts[i].post_id != null) {
							$(".post").append(
							...
							if (user != null) {
								$(".a" + page + i + "").append(
										"<a href=\"#\" onclick=\"return false;\" class=\"iconinvisible like\" title=\"按個「讚」！\"><img id=\"l" + json.posts[i].post_id + "\" src=\"" + imgURL + "images/like.png\"></a>" +
										"<a href=\"#\" onclick=\"return false;\" class=\"iconinvisible favorite\" title=\"將這則貼文加入最愛\"><img id=\"f" + json.posts[i].post_id + "\" src=\"" + imgURL + "images/favorite.png\"></a>");
								if (user_id == json.posts[i].m_id) {
									$(".a" + page + i + "").append(
											"<a href=\"#\" onclick=\"return false;\" class=\"iconinvisible edit\" title=\"編輯這則貼文\"><img id=\"ep" + json.posts[i].post_id + "\" src=\"" + imgURL + "images/edit.png\"></a>" +
											"<a href=\"#\" onclick=\"return false;\" class=\"iconinvisible delete\" title=\"刪除這則貼文\"><img id=\"dp" + json.posts[i].post_id + "\" src=\"" + imgURL + "images/delete.png\"></a>");
								} else {
									$(".a" + page + i + "").append(
									"<a href=\"#\" onclick=\"return false;\" class=\"newreport iconinvisible\" title=\"檢舉這則貼文\"><img id=\"a" + json.posts[i].post_id + "\" src=\"" + imgURL + "images/report.png\"></a>");
								}
							} else {
								$(".a" + page + i + "").append(
										"<a href=\"#\" onclick=\"return false;\" class=\"newwarning iconinvisible\" title=\"發送警告\"><img id=\"w" + json.posts[i].post_id + "\" src=\"" + imgURL + "images/warn.png\"></a>");
							}
							var whoLikeName = "";
							var whoLikeSum = json.posts[i].like_count;
							if (json.whoLikes != null) {
								var count = 0;
								for (var l = 0; l < json.whoLikes.length; l++) {
									if (json.whoLikes[l].post_id == json.posts[i].post_id && count == whoLikeSum - 1) {
										whoLikeName += json.whoLikes[l].m_name;
									} else if (json.whoLikes[l].post_id == json.posts[i].post_id) {
										whoLikeName += json.whoLikes[l].m_name + "、";
										count++;
									}
								}
							...
							$(".b" + page + i + "").append(
									"</div>" +
									"<div class=\"likeblock\">" +
									"<a href=\"#\" id=\"wl" + json.posts[i].post_id + "\" onclick=\"return false;\" class=\"liketext\" title=\"誰按讚：" + whoLikeName + "\">" + json.posts[i].like_count + "</a>" +
									"<img src=\"" + imgURL + "images/like_arrow.png\" style=\"opacity: 0.4; filter: alpha(opacity = 40); \">"+
									"</div>");
							if (json.posts[i].m_pic_path == null) {
								$(".b" + page + i + "").append(
										"<a href=\"#\" onclick=\"return false;\" style=\"text-decoration: none;\"><img src=\"" + imgURL + "images/cat1.jpg\" class=\"memberphoto\"><span class=\"membername\">" + json.posts[i].m_name + "</span></a><span style='color: grey;'>" + jQuery.timeago(json.posts[i].post_time) + "</span><br/>");
							} else {
								$(".b" + page + i + "").append(
										"<a href=\"#\" onclick=\"return false;\" style=\"text-decoration: none;\"><img src=\"" + imgURL + "userImages/" + json.posts[i].m_id + "/" + json.posts[i].m_pic_path + "\" class=\"memberphoto\"><span class=\"membername\">" + json.posts[i].m_name + "</span></a><span style='color: grey;'>" + jQuery.timeago(json.posts[i].post_time) + "</span><br/>");
							}
							$(".b" + page + i + "").append(
									"<div class=\"postcontent\">" +
									json.posts[i].post_content + "<br><br>" +
									"<div class=\"waterwheelcarousel\">" +
									"<div class=\"d" + page + i + "\">" +
									"</div>");
							if (json.pics != null) {
								for (var j = 0; j < json.pics.length; j++) {
									if (json.pics[j].post_id == json.posts[i].post_id) {
										$(".d" + page + i + "").append(
							...
							$(".c" + page + i + "").append(
									...
									"<textarea cols=\"52\" rows=\"0\" placeholder=\"我要留言\" class=\"respondtextarea text ui-widget-content ui-corner-all\"></textarea><a href=\"#\" onclick=\"return false;\" id=\"r" + json.posts[i].post_id + "\" class=\"buttonClass summitresponse\">送出</a><br/>" +
									"</div>");
							if (json.res != null) {
								for (var k = 0; k < json.res.length; k++) {
									if (json.res[k].post_id == json.posts[i].post_id) {
										$(".c" + page + i + "").append(
												...
												"<div class=\"e" + page + k + "\">");
										if (user_id == json.posts[i].m_id || user_id == json.res[k].m_id) {
											$(".e" + page + k + "").append(
		...
		"complete" : function() {
			...
			$(".liketext").unbind( "click" ).click(function() {
			...
			//發送警告
			$('.newwarning').on('click',$(this).children("img").attr("id"),function() {
				var post_id = $(this).children("img").attr("id").slice(1);
				parent.parent.parent.parent.$("#newwarningiframe").attr("src", imgURL + "pages/NewWarning.jsp?post_id=" + post_id);
			});
		}
	});
});
			...
			if(data != "") {
				...
				if (json.posts != "") {
					...
					for(var i = 0; i < length; i++) {
						if (json.posts[i].post_id != null) {
							$(".post").append(
									"<div class=\"page" + page + i + "\" id=\"" + json.posts[i].post_id + "\">" +
									"<div class=\"scroll\" style=\"width: 700px;\">" +
									"<div class=\"outoutblock\">" +
									"<a href=\"#\" class=\"outblockspand\"><img src=\"" + imgURL + "images/post-arrow-down.png\" class=\"spandarrow\"></a>" +
									"<div class=\"outblock\">" +
									"<div class=\"c" + page + i + "\" id=\"c" + json.posts[i].post_id + "\">" +
									"<div class=\"block\">" +
									"<div class=\"b" + page + i + "\">" +
									"<div class=\"manyicon\">" +
									"<div class=\"a" + page + i + "\">");
